...
// requiring a repost. Also stamps the poster's *current* hidePhone value
// onto every ad as `_hidePhone` — that flag must always reflect live
// state (a hirer flipping it on must immediately stop showing their
// number on every one of their ads, not just future ones), so unlike
// the logo it's re-checked for every ad, not just ones missing something.
// Requires the viewer to be signed in (hirer_profiles reads are
// auth-gated) — anonymous visitors silently keep initials and, since the
// Contact button itself is behind a sign-in gate before any phone number
// is ever shown, never reach a point where a stale/missing _hidePhone
// could leak a number the hirer asked to keep hidden.
const _enrichFromHirerProfile = async (ads) => {
  const uids = [...new Set(ads.filter(a => a.userId).map(a => a.userId))];
  ...
  for (let i = 0; i < uids.length; i += 10) {
    ...
    } catch { /* not signed in, or rules deny — leave this batch unenriched */ }
  }
  ads.forEach(a => {
    const profile = profiles.get(a.userId);
    if (!profile) return;
    if (!a.posterLogo && profile.companyLogoURL) a.posterLogo = profile.companyLogoURL;
    a._hidePhone = profile.hidePhone === true;
  });
};
...
export const getAds = async ({ limitCount = 50 } = {}) => {
  ...
  const snap = await getDocs(q);
  const docs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  await _enrichFromHirerProfile(docs);
