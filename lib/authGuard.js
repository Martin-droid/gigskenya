...
/**
 * Send a signed-in hirer straight to the Dashboard's post tab, short-
 * circuiting whatever real href the calling <Link> points at (the SEO
 * landing page for a fresh visitor, /register for a logged-out click).
 * Pass the click event so real <a href> markup stays crawlable — only
 * authenticated users ever get intercepted; everyone else just follows
 * the link normally.
 */
export function goToPostJob(navigate, isAuthenticated, e) {
  if (!isAuthenticated) return;
  e?.preventDefault?.();
  navigate('/dashboard', { state: { tab: 'post' } });
}
