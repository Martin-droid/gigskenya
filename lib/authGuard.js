...
/**
 * Send the user straight into the "post a job" flow: signed-in hirers
 * land directly on the Dashboard's post tab, everyone else goes through
 * registration first (Dashboard's own onboarding gate takes it from there).
 */
export function goToPostJob(navigate, isAuthenticated) {
  if (isAuthenticated) navigate('/dashboard', { state: { tab: 'post' } });
  else navigate('/register?role=hirer');
}
